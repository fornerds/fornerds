...
import { Button, Input } from '../../AtomComponent'
...
export function Comment({
  ...
  const [editingCommentId, setEditingCommentId] = useState<{
    ...
    replyId: number | null
  }>({ commentId: null, replyId: null }) // 수정시 대상의 ID확인
  ...
  const handleEditClick = (
    ...
    setEditedContent(
      replyId
        ? commentData.replyData.find((reply) => reply.id === replyId)
            ?.content || ''
        : commentData.content
    )
  ...
  const handleSaveClick = () => {
    ...
    if (editingCommentId.replyId) {
      const updatedReplyData = commentData.replyData.map((reply) =>
        reply.id === editingCommentId.replyId
    ...
    <div className={styles.discussion}>
      <div className={styles.comment}>
        <div className={styles.nameBox}>
          <img
            src={characters[currentUser.profileImage]}
          ...
          {!hideEditDelete && isCurrentUser(commentData.user_id) && (
            <div className={styles.editBox}>
              <Button
                ...
                onClick={() => handleEditClick(commentData.id)}
              >
                <div className={styles.editIcon}>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none">
                    <path
                      d="M13.9808 2.80618C14.1843 2.56552 14.4359 2.36964 14.7197 2.231C15.0035 2.09236 15.3132 2.01403 15.6291 2.00097C15.9451 1.98792 16.2603 2.04045 16.5546 2.1552C16.849 2.26995 17.1161 2.44441 17.3389 2.66747C17.5617 2.89052 17.7353 3.15727 17.8487 3.45071C17.9621 3.74416 18.0127 4.05786 17.9973 4.37183C17.982 4.6858 17.901 4.99314 17.7595 5.27427C17.6181 5.5554 17.4193 5.80416 17.1757 6.00468L6.39298 16.7996L2 17.999L3.19808 13.6011L13.9808 2.80618Z"
                      stroke="#1890FF"
                      strokeWidth="1.152"
                      strokeLinecap="round"
        ...
        {editingCommentId.commentId === commentData.id &&
          ...
          <div className={styles.interactions}>
            <div className={styles.replyBox}>
              ...
              <div className={styles.replyText}>
                <Button
                  className={`${styles.reply} text-color-default font-roboto-tag`}
        ...
        <div className={styles.replys}>
          {commentData.replyData.map((reply) => (
            <div className={styles.comment} key={reply.id}>
              <div className={styles.nameBox}>
                ...
                {!hideEditDelete && isCurrentUser(reply.user_id) && (
                  <div className={styles.editBox}>
                    <Button
                      className={styles.edit}
                      onClick={() => handleEditClick(commentData.id, reply.id)}
                    >
                      <div className={styles.editIcon}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none">
                          <path
              ...
              {editingCommentId.commentId === commentData.id &&
              editingCommentId.replyId === reply.id ? (
                <div className={styles.comment}>
                  <div className={styles.nameBox}>
                    <img
                      src={characters[currentUser.profileImage]}
